...
_set_default(
    ...
    os.getenv("MARKETLIFT_COMMERCE_PROVIDER", "pagarme").strip(),
)
_set_default("PAGARME_SECRET_KEY", os.getenv("PAGARME_SECRET_KEY", "").strip())
_set_default(
    "PAGARME_BASE_URL",
    os.getenv("PAGARME_BASE_URL", "https://api.pagar.me/core/v5").strip(),
)
_set_default(
    "PAGARME_MARKETPLACE_RECIPIENT_ID",
    os.getenv("PAGARME_MARKETPLACE_RECIPIENT_ID", "").strip(),
)
_set_default("PAGARME_WEBHOOK_TOKEN", os.getenv("PAGARME_WEBHOOK_TOKEN", "").strip())
_set_default(
    "PAGARME_TIMEOUT_SECONDS", float(os.getenv("PAGARME_TIMEOUT_SECONDS", "15"))
)
_set_default(
    "PAGARME_PIX_EXPIRES_SECONDS", int(os.getenv("PAGARME_PIX_EXPIRES_SECONDS", "1800"))
)
_set_default(
    "PAGARME_STATEMENT_DESCRIPTOR",
    os.getenv("PAGARME_STATEMENT_DESCRIPTOR", "MARKETLIFT").strip(),
)
_set_default(
    "MARKETLIFT_COMMERCE_FEE_BPS", int(os.getenv("MARKETLIFT_COMMERCE_FEE_BPS", "500"))
)
_set_default(
...
_services.withdraw_available_balance = _review_fixes.withdraw_available_balance
