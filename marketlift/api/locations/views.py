    ...
    def get(self, request):
        ...
        try:
            cities = brazil_cities(
                ...
                limit=_limit(
                    request.query_params.get("limit"), default=80, maximum=200
                ),
            )
        except DjangoValidationError as exc:
            raise _drf(exc) from exc
...
class LocationSearchView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            raise _drf(exc) from exc
        rows = [row for row in rows if (row.country_code or "").upper() == "BR"]
        return Response(
...
class LocationReverseView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            raise _drf(exc) from exc
        if row is None or (row.country_code or "").upper() != "BR":
